...
export const withSVGStylerHOC = <P extends classNameProp>(
  ...
  const WrappedComponent = ({ className, ...rest }: P) => {
    return (
      <Component
        className={`w-5 h-5 mr-1 stroke-stone-400 hover-text before:w-24 before:text-stone-500 before:bg-stone-300 before:tracking-wide before:shadow-md text-sm border-stone-300 fill-stone-400 ${className}`}
        {...(rest as P)}
      />
    );
  };
